...
if __name__ == '__main__':
    """
    Script used to make a lagle sumbmission  for a decision tree
    """
    ...
    # Best columns from FeatureSelection.py
    X_ls = X_ls[['release_date', 'Comedy', 'Drama', 'Horror', 'War']]
    ...
    with measure_time('Training'):
        ...
        model = DecisionTreeRegressor(max_depth = dpt)
        model.fit(X_ls, y_ls)

    # ------------------------------ Prediction ------------------------------ #
    X_ts = make_test_set()
    X_ts2 = X_ts[['release_date', 'Comedy', 'Drama', 'Horror', 'War']]
    ...
    for index, item in X_ts.iterrows():
        user = item['user_id']
        movie = item['movie_id']
        xSubmit.append([user, movie])

    fname = make_submission(y_pred, xSubmit, 'DTBestFeatures')
