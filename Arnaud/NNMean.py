...
def make_dataset(toSplit = False, splitRatio = 0.2):
	...
	userStdRating = trainDataset.groupby("user_id", as_index = False).rating.std()
	userStdRating.rename(index = str, columns = {"rating": "userStdRating"}, inplace = True)
	movieStdRating = trainDataset.groupby("movie_id", as_index = False).rating.std()
	...
	trainDataset = pd.merge(trainDataset, userStdRating, on = "user_id")
	trainDataset = pd.merge(trainDataset, movieStdRating, on = "movie_id")
	...
	testDataset = pd.merge(testDataset, userStdRating, on = "user_id")
	testDataset = pd.merge(testDataset, movieStdRating, on = "movie_id")
